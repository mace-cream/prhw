...
if Network=='MLP':
    ...
    model = MLP(config)
if Network=='LeNet':
    config = {'EpochNum': 100, 'BatchSize': 100, 'Height': 28, 'Width': 28, 'InputDim': 1, 'OutputDim': 10, 'LayerNum': 3, 'HiddenNum': [32,64,128], 'KernelSize': 3, 'LearningRate': 0.001, 'lambda': 0.0}
    model = LeNet(config)
